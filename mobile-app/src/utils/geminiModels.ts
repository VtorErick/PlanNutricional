...
const LEGACY_GEMINI_API_KEY_STORAGE_KEY = '@app:geminiApiKey';
const GEMINI_MODEL_STORAGE_KEY = '@app:geminiModel';
...
export async function getStoredGeminiModel() {
  const saved = await readStorageValue(GEMINI_MODEL_STORAGE_KEY);
  const normalized = normalizeModelName(saved || '');
  return isSupportedGeminiTextModel(normalized) ? normalized : DEFAULT_GEMINI_MODEL;
}

export async function persistGeminiModel(modelName: string) {
  const normalized = normalizeModelName(modelName);
  if (!normalized) {
    await removeStorageValue(GEMINI_MODEL_STORAGE_KEY);
    return;
  }

  await writeStorageValue(GEMINI_MODEL_STORAGE_KEY, normalized);
}

export async function clearLegacyGeminiApiKeyStorage() {
  await removeStorageValue(LEGACY_GEMINI_API_KEY_STORAGE_KEY);
}
