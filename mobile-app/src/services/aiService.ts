...
export async function callGeminiDirectly(
  ...
  if (!targetProfile || !['el', 'ella', 'ambos'].includes(targetProfile)) {
    throw new Error('targetProfile invalido. Debe ser: el, ella o ambos.');
  }

  const normalizedApiKey = String(apiKey || '').trim();
  if (!normalizedApiKey) {
    throw new Error('Falta EXPO_PUBLIC_GEMINI_API_KEY para usar IA directa en mobile.');
  }

  const requestedModel =
    normalizeModelName(modelName || payload?.preferredModel || DEFAULT_DIRECT_MODEL) ||
    DEFAULT_DIRECT_MODEL;
  const isRevision = isPlanRevisionRequest(payload);
  const requestMode: 'generate' | PlanRevisionMode = isRevision ? payload.requestMode : 'generate';
  const flow: AiDebugLog['flow'] = isRevision ? 'plan-revision' : 'questionnaire-submit';
  ...
  const models = await listAvailableModelsDirect(normalizedApiKey, debugBase);
  const selectedModel = pickBestModel(models, requestedModel);
  const fallbackModels = getFallbackModels(models, selectedModel);
  const modelCandidates = [selectedModel, ...fallbackModels].slice(0, MAX_MODEL_CANDIDATES);

  let elData: Record<string, unknown> | PlanRevisionProfilePatch | null = null;
  let ellaData: Record<string, unknown> | PlanRevisionProfilePatch | null = null;
  ...
  if (targetProfile === 'el' || targetProfile === 'ambos') {
    const payloadEl = targetProfile === 'ambos' ? buildScopedPayload(payload, payload.el) : payload;
    const result = await generateContentWithFallback(
    ...
    elData = result.data as Record<string, unknown>;
    elModelUsed = result.modelUsed;
  }

  if (targetProfile === 'ella' || targetProfile === 'ambos') {
    const payloadElla = targetProfile === 'ambos' ? buildScopedPayload(payload, payload.ella) : payload;
    if (targetProfile === 'ambos' && (elData as any)?.planEL) {
      (payloadElla as any).companionPlan = (elData as any).planEL;
    }
    const result = await generateContentWithFallback(
    ...
    ellaData = result.data as Record<string, unknown>;
    ellaModelUsed = result.modelUsed;
  }

  const modelUsed = Array.from(new Set([elModelUsed, ellaModelUsed].filter(Boolean))).join(', ');
  return { elData, ellaData, modelUsed };
}
