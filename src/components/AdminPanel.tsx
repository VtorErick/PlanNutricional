  ...
  return (
    <div className={`p-5 rounded-2xl border ${borderColor} bg-gradient-to-br ${bgGradient} shadow-sm flex flex-col gap-4 relative overflow-hidden`}>
      ...
      <div className="flex flex-col gap-3 z-10">
        ...
        {/* Opciones de carga */}
        {isCustomAvailable ? (
          <div className="flex items-center justify-between gap-1 sm:gap-2 p-3 bg-white rounded-xl border border-slate-200 shadow-sm">
            <span className="text-xs sm:text-sm font-medium text-slate-600">Usar versión:</span>
            <button 
              onClick={() => setDataVersion(dataVersion === 'original' ? 'custom' : 'original')}
              className={`flex items-center gap-1 sm:gap-2 font-bold text-xs sm:text-sm transition-colors ${dataVersion === 'custom' ? activeColor : 'text-slate-400'}`}
            >
