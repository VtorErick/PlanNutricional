          ...
          <motion.div
            ...
            className={`relative overflow-hidden rounded-2xl sm:rounded-[26px] transition-all duration-300 text-left w-full group cursor-pointer ${
              esSeleccionada
                ...
                : isDarkMode
                  ? 'bg-ink-900/92 border border-ink-700 shadow-[0_12px_28px_rgba(2,6,23,0.35)] hover:border-ink-600 hover:shadow-[0_16px_34px_rgba(2,6,23,0.45)]'
                  : 'bg-white border border-cream-100/90 shadow-sm hover:border-cream-200 hover:shadow-md'
            }`}
          >
            {esSeleccionada ? (
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <h4 className={`font-black text-[15px] sm:text-base tracking-tight leading-snug break-words ${isDarkMode ? 'text-cream-100' : 'text-ink-700'}`}>
                          {comida.nombre}
                        </h4>
                      </div>
                      ...
                      <div
                        className={`w-7 h-7 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all duration-300 ${
                          esSeleccionada
                            ...
                            : isDarkMode
                              ? 'border-ink-600 group-hover:border-ink-400 bg-ink-800'
                              : 'border-cream-200 group-hover:border-cream-300 bg-cream-50'
                        }`}
                      >
                        {esSeleccionada ? (
                          <motion.div
              ...
              <div className="pt-0.5">
                {esSeleccionada && porciones.length > 0 ? (
                  ...
                  >
                    ...
                    <span className={`font-medium tracking-tight leading-relaxed break-words ${isDarkMode ? 'text-cream-100' : 'text-ink-600'}`}>
                      {comida.porciones}
                    </span>
                  </div>
                )}
