...
import { CheckCircle2, ChevronUp, Zap, UtensilsCrossed, AlertTriangle, FileText, Sun, Apple, Coffee, Moon } from 'lucide-react';
...
export default function PlanView() {
  const { 
    perfilActivo, perfilBase, perfilesData, diaActivo, isAmbos, 
    selecciones, toggleSeleccion, 
    momentosEnEdicion, setMomentosEnEdicion, 
    momentosColapsados, setMomentosColapsados, 
    momentoCompletado, progresoDia, ac,
    mealSectionRefs
  } = useDiet();
    ...
    <motion.div key="plan"
      ...
      <div className="space-y-4">
        {perfilBase.momentos.map((momento) => {
          ...
          const porcionesSingleMomento = !isAmbos && perfilActivo && perfilActivo !== 'ambos' ? getMomentMacroPortions(perfilesData[perfilActivo], momento.key) : [];
          ...
          return (
            <motion.div
              layout
              key={momento.key}
              ref={(el) => { if (el) mealSectionRefs.current[momento.key] = el; }}
      ...
      {progresoDia === 100 && (
        <motion.div 
          ...
          className={`mt-8 p-6 lg:p-8 rounded-[2rem] bg-gradient-to-br ${ac.bgGradient} text-white shadow-xl ${ac.shadowLight} relative overflow-hidden flex flex-col sm:flex-row items-center justify-between gap-6`}
        >
